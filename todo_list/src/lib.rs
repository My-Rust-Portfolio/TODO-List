...
mod tests {
    ...
    fn test_app_state_add_command() {
        let mut state = AppState { tasks: vec![], next_index: 0 };
        handle_command(&mut state, &Commands::Add { title: "Task 1".to_string(), description: "Description 1".to_string() });
    ...
    fn test_app_state_next_id() {
        let mut state = AppState { tasks: vec![], next_index: 0 };
        handle_command(&mut state, &Commands::Add { title: "Task 0".to_string(), description: "Description 0".to_string() });
        ...
        state = load_state_from_file_name(file_name).expect("Failed to load state");
        handle_command(&mut state, &Commands::Add { title: "Task 2".to_string(), description: "Description 2".to_string() });
        ...
        state = load_state_from_file_name(file_name).expect("Failed to load state");
        handle_command(&mut state, &Commands::Add { title: "Task 3".to_string(), description: "Description 3".to_string() });
