...
mod tests {
    ...
    use crate::persistence::{delete_state_file_name, load_state_from_file_name, save_state_to_file_name, state_file_name};
    use crate::command_handler::{self, Commands, handle_command};
    ...
    fn test_app_state_add_command() {
        let mut state = AppState { tasks: vec![], next_index: 0 };
        command_handler::handle_command(&mut state, &Commands::Add { title: "Task 1".to_string(), description: "Description 1".to_string() });
    ...
    fn test_app_state_complete_command() {
        let mut state = AppState { tasks: vec![], next_index: 0 };
        command_handler::handle_command(&mut state, &Commands::Add { title: "Task 1".to_string(), description: "Description 1".to_string() });
        command_handler::handle_command(&mut state, &Commands::Complete { index: 0 });
    ...
    fn test_app_state_delete_command() {
        let mut state = AppState { tasks: vec![], next_index: 0 };
        command_handler::handle_command(&mut state, &Commands::Add { title: "Task 1".to_string(), description: "Description 1".to_string() });
        command_handler::handle_command(&mut state, &Commands::Delete { index: 0 });
    ...
    fn test_app_state_next_id() {
        let mut state = AppState { tasks: vec![], next_index: 0 };
        command_handler::handle_command(&mut state, &Commands::Add { title: "Task 0".to_string(), description: "Description 0".to_string() });
        let file_name = "test_state.json";
        save_state_to_file_name(&state, file_name).expect("Failed to save state");
        ...
        state = load_state_from_file_name(file_name).expect("Failed to load state");
        command_handler::handle_command(&mut state, &Commands::Add { title: "Task 2".to_string(), description: "Description 2".to_string() });
        assert_eq!(state.tasks[2].index, 2);
        save_state_to_file_name(&state, file_name).expect("Failed to save state");

        state = load_state_from_file_name(file_name).expect("Failed to load state");
        command_handler::handle_command(&mut state, &&Commands::Delete { index: (1) });
        assert_eq!(state.tasks[1].index, 2);
        save_state_to_file_name(&state, file_name).expect("Failed to save state");

        state = load_state_from_file_name(file_name).expect("Failed to load state");
        command_handler::handle_command(&mut state, &Commands::Add { title: "Task 3".to_string(), description: "Description 3".to_string() });
